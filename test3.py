    ...
    while(1):
      for i in range(1, n):
          ...
          for ii in range(0, classnumber):
              ...
              Groupsum = Groupsum + (IGg[ii,i] * np.linalg.norm(group)) / np.linalg.norm(W[i,:])
      ...
      for i in range(1, n):
          dn[i] = np.sqrt(sum((np.sum(np.multiply(W,W), 2-1) + eps)))/np.sum(W[i,:])
      ...
      W21 = np.sum(Wi)
      Wd = np.sum(W,1)
      obj.append(np.linalg.norm(Y.T - np.dot(X.T, W),'fro')** 2 + alpha * W21 + beta * np.dot(Wd.T, np.dot(F, Wd)))
